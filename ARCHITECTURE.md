...
**Persistence:** after every mutation (`session_register`, `canvas_append`, `canvas_clear`) the full session list is marshaled to `~/.local/share/tui-canvas/sessions.json`. On startup the daemon reloads this file so sessions survive daemon restarts.
...
- `full_state` → replace all sessions (initial sync)
...
│                                                   │
├───────────────────────────────────────────────────┤
│  tab/shift+tab: switch  ↑↓/jk: scroll  ?: id  q  │  1 line
└───────────────────────────────────────────────────┘
```

**Keybindings:** `tab`/`shift+tab` switch sessions, `↑↓`/`j`/`k` scroll, `?` toggles debug mode (shows session IDs), `q`/`ctrl+c` quit.
...
The emitted string `[tui-canvas] Your session ID is: <UUID>` is what skills look for in context. If the context is lost (e.g., after `/clear`), skills fall back to `tui-canvas list --cwd "$(pwd)"`.

### Skills

**`/tui-canvas`** — sends content to the canvas. Resolution order for session ID:
1. Scan context for `[tui-canvas] Your session ID is: <UUID>`
2. Run `tui-canvas list --cwd "$(pwd)"` and take the first result
3. Fail silently with a hint to press `?` in the TUI

Sends a `canvas_append` message via `tui-canvas send` (reads JSON from stdin, writes to socket).

**`/tui-clear`** — sends `canvas_clear` for the current session. Uses the same session ID resolution as `/tui-canvas`.
